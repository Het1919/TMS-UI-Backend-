...
const ShowTenantWithAggregatorAndBanks: React.FC = () => {
  ...
  const specificTenantName = useParams();

  // console.log(specificTenantName);
    ...
    return (
      <div>
        {Object.entries(groupedData).map(([tenantName, tenantData]) => {
          // Check if the tenantId matches the current tenantName
          if (tenantName === specificTenantName.tenantName) {
            return (
          ...
          return null; // Return null for tenants that don't match the selectedTenantId
        })}
      </div>
    );
  }
